        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start">
              ...
              <div className="space-y-6 mb-10">
                {DIFFERENTIATORS.map((d) => (
                  <div key={d.label} className="flex gap-4">
                    <div className="flex-shrink-0 w-px bg-teal/30 self-stretch" />
                    <div>
                      <p className="text-slate-900 font-semibold mb-1">{d.label}</p>
          ...
          <AnimateOnView delay={0.12} direction="left">
            <div>
              ...
              <div className="space-y-0 divide-y divide-paper-border border-y border-paper-border">
                {OUTCOMES.map((o) => (
                  <div key={o.stat} className="flex items-start gap-6 py-7">
                    <span className="font-display text-display-sm font-bold text-slate-900 flex-shrink-0 w-20">
                      {o.stat}
                    </span>
                    <p className="text-slate-500 leading-relaxed pt-1">{o.desc}</p>
                  </div>
                ))}
              </div>

              <blockquote className="mt-10 pl-5 border-l-2 border-teal/20">
